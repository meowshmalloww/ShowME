...
Rust test result: 16 tests passed. Coverage targets include capture crop mapping, region and URL validation, lesson schema and citation reconciliation, provider parsing/defaults, request privacy and context opt-ins, Wikimedia license/metadata handling, SQLite settings/memory behavior, model-route construction, and scale-aware collapsed/expanded pet window sizing.

The Windows production build budget passed with 206.2 KiB of initial JavaScript (67.3 KiB gzip), 84.2 KiB of initial CSS (19.3 KiB gzip), and 356.9 KiB of JavaScript across all deferred routes. The build fails if startup or total bundle limits regress. The production build contains no sample-lesson or fake browser-capture route.
...
- transparent compact pet, capture/mic/menu actions, and a clean native background edge;
- native full-screen capture, rectangle selection, selection count, and crop commit;
- direct capture handoff to the expanded pet with a real crop thumbnail;
- compact text/microphone entry, per-request context switches, no-key send gating, and Settings recovery;
- centralized cards for OpenAI, NVIDIA NIM, Groq, Cerebras, and OpenRouter keys;
- teaching settings including saved 80–145% pet sizing;
...
The `0.2.0` pass was exercised at the normal desktop viewport, the app’s minimum `940 × 620` window, the collapsed `208 × 226` pet, and the expanded request panel. It covered the mothership home, lesson workspace, expanded visualization, library recovery actions, provider/teaching settings, pet/menu states, capture-to-pet transitions, and the application-controlled reduced-motion class.
...
The optimized Windows executable was built successfully and launched directly from the release directory. The pass verified process startup, native transparency, the pet menu, the real capture overlay, rectangle drawing, crop commit, pet expansion, crop dimensions/preview, no-key generation gating, provider settings, and pet sizing controls. The Windows installer payloads were generated by Tauri’s NSIS and WiX pipelines.
...
- Live OpenAI, NVIDIA, Groq, Cerebras, or OpenRouter requests: no user credentials were available, so success was not simulated or claimed.
...
2. Complete onboarding and verify ShowME appears in the tray and the pet can be moved.
